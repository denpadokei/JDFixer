...
namespace JDFixer
{
    ...
    internal class SpawnMovementDataUpdatePatch
    {
        internal static void Prefix(ref float startNoteJumpMovementSpeed, float startBpm, /*ref float noteJumpStartBeatOffset,*/ ref BeatmapObjectSpawnMovementData.NoteJumpValueType noteJumpValueType, ref float noteJumpValue)
        {
            ...
            var noteJumpStartBeatOffset = noteJumpValue;

            var mapNJS = startNoteJumpMovementSpeed;
            ...
            // 1.19.1
            var desiredJumpDis = PluginConfig.Instance.slider_setting == 0
                ? PluginConfig.Instance.jumpDistance
                : PluginConfig.Instance.reactionTime * mapNJS / 500;
            ...
            {
                desiredJumpDis = PluginConfig.Instance.slider_setting == 0 ? BeatmapOffsets.jd_snap_value : BeatmapOffsets.rt_snap_value * mapNJS / 500;
            }
        ...
        SongSpeed:
            ...
            var jumpDurCurr = num2Curr * numCurr * 2f;
            var jumpDisCurr = mapNJS * jumpDurCurr;

            var desiredJumpDur = desiredJumpDis / mapNJS;
            var desiredHalfJumpDur = desiredJumpDur / 2f / num2Curr;
            var jumpDurMul = desiredJumpDur / jumpDurCurr;
    ...
    [HarmonyPatch(typeof(MissionSelectionMapViewController), nameof(MissionSelectionMapViewController.SongPlayerCrossfadeToLevelAsync))]
    ...
    internal class StandardLevelScenesTransitionSetupDataSOPatch
    {
        ...
        {
            foreach (var method in typeof(StandardLevelScenesTransitionSetupDataSO).GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Where(x => x.Name == nameof(StandardLevelScenesTransitionSetupDataSO.Init)))
            {
                var args = method.GetParameters().Select(x => x.ParameterType).ToArray();
                if (!args.Contains(typeof(GameplayModifiers))
                    || !args.Contains(typeof(PracticeSettings)))
                {
                    continue;
                }
                yield return method;
    ...
    internal class SpawnMovementDataUpdateHelper
    {
        ...
        {
            var new_RT = BeatmapUtils.Calculate_ReactionTime_Setpoint_Float(jumpDis, mapNJS) * BeatmapInfo.speedMultiplier;

            return PluginConfig.Instance.song_speed_setting == 1
                ? BeatmapUtils.Calculate_JumpDistance_Setpoint_Float(new_RT, mapNJS)
                : PluginConfig.Instance.song_speed_setting == 2 &&
    ...
    {
        private static readonly DateTime af = new DateTime(DateTime.Now.Year, 4, 1);
        internal static BeatmapObjectSpawnMovementData.NoteSpawnData Postfix(BeatmapObjectSpawnMovementData.NoteSpawnData __result)
        {
            if (DateTime.Now >= af && TimeController.audioTime.songTime >= 5f)
            {
                ...
                var jumpDuration = (float)(__result.jumpDuration * (1 + (0.30 * Math.Abs(Math.Sin(4 * Math.PI * (TimeController.audioTime.songTime - 5f) / TimeController.length)))));
